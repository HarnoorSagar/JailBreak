...
namespace Final_Project_Day_1
{
    public partial class Form1 : Form
    {
        ...
        //declares the scoore textbox variable
        private TextBox txtboxScore;

        //declares the highscore textbox variables
        int highscore;
        private TextBox txtboxHighscore;

        //creates a picbox for the you win screen
        Image youWin;
        private PictureBox picboxWin;

        //creates a picbox for the you win screen
        Image youLose;
        private PictureBox picboxLose;

        //declares the game textbox variable
        private TextBox txtboxGameTimer;

        //declares the game timer variable
        int gameTimer = 45;

        //declares the time refreshed variable
        int timeRefreshed = 0;

        //declares the time passed variable
        int timePassed = 0;

        //declares the streamwriter variable
        StreamWriter fileWriter;

        //declares the streamreader variable
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //******************picboxWin PROPERTIES**********************

            //initializes the picture box
            picboxWin = new PictureBox();

            //gets the you win image
            youWin = Image.FromFile(Application.StartupPath + @"\YOU WIN.png");

            //changes the location of the textbox
            picboxWin.Location = new Point(700, 150);

            //ensures that the picture box is visible
            picboxWin.Visible = true;

            //changes the size of the textbox
            picboxWin.Size = new Size(540, 720);

            //adds the image to the picbox
            picboxWin.Image = youWin;

            //removes the picbox border
            picboxWin.BorderStyle = BorderStyle.None;

            //******************picboxLose PROPERTIES**********************

            //initializes the picture box
            picboxLose = new PictureBox();

            //gets the you lose image
            youLose = Image.FromFile(Application.StartupPath + @"\YOU LOSE.png");

            //changes the location of the textbox
            picboxLose.Location = new Point(700, 150);

            //ensures that the picture box is visible
            picboxLose.Visible = true;

            //changes the size of the textbox
            picboxLose.Size = new Size(540, 720);

            //adds the image to the picbox
            picboxLose.Image = youLose;

            //removes the picbox border
            picboxLose.BorderStyle = BorderStyle.None;

            //******************txtboxScore PROPERTIES**********************

            //initializes the textbox
            txtboxScore = new TextBox();

            //makes the textbox read only
            txtboxScore.ReadOnly = true;

            //makes enabled false
            txtboxScore.Enabled = false;

            //changes the location of the textbox
            txtboxScore.Location = new Point(750, 0);

            //changes the color of the textbox
            txtboxScore.BackColor = Color.Gold;

            //changes the font colour
            txtboxScore.ForeColor = Color.Black;

            //makes autosize of the textbox false
            this.txtboxScore.AutoSize = false;

            //changes the size of the textbox
            txtboxScore.Size = new Size(145, 20);

            //adds text to the textbox
            txtboxScore.Text = "Bags Collected: " + bagsCollected;

            //changes the font of the textbox
            txtboxScore.Font = new Font("Arial", 12, FontStyle.Regular);

            //centers the textbox text
            txtboxScore.TextAlign = HorizontalAlignment.Center;

            //removes the textbox border
            txtboxScore.BorderStyle = BorderStyle.None;

            //adds the textbox on the screen
            this.Controls.Add(txtboxScore);

            //******************txtboxHighscore PROPERTIES**********************

            //initializes the textbox
            txtboxHighscore = new TextBox();

            //makes the textbox read only
            txtboxHighscore.ReadOnly = true;

            //makes enabled false
            txtboxHighscore.Enabled = false;

            //changes the location of the textbox
            txtboxHighscore.Location = new Point(1050, 0);

            //changes the color of the textbox
            txtboxHighscore.BackColor = Color.Gold;

            //changes the font colour
            txtboxHighscore.ForeColor = Color.Black;

            //makes autosize of the textbox false
            this.txtboxHighscore.AutoSize = false;

            //changes the size of the textbox
            txtboxHighscore.Size = new Size(125, 20);

            //changes the font of the textbox
            txtboxHighscore.Font = new Font("Arial", 12, FontStyle.Regular);

            //centers the textbox text
            txtboxHighscore.TextAlign = HorizontalAlignment.Center;

            //removes the textbox border
            txtboxHighscore.BorderStyle = BorderStyle.None;

            //adds the textbox on the screen
            this.Controls.Add(txtboxHighscore);

            //******************gameTimer PROPERTIES**********************

            //initializes the textbox
            txtboxGameTimer = new TextBox();

            //makes the textbox read only
            txtboxGameTimer.ReadOnly = true;

            //makes enabled false
            txtboxGameTimer.Enabled = false;

            //changes the location of the textbox
            txtboxGameTimer.Location = new Point(930, 0);

            //changes the color of the textbox
            txtboxGameTimer.BackColor = Color.Snow;

            //changes the font colour
            txtboxGameTimer.ForeColor = Color.Black;

            //makes autosize of the textbox false
            this.txtboxGameTimer.AutoSize = false;

            //changes the size of the textbox
            txtboxGameTimer.Size = new Size(80, 20);

            //adds text to the textbox
            txtboxGameTimer.Text = "Timer: " + gameTimer;

            //changes the font of the textbox
            txtboxGameTimer.Font = new Font("Arial", 12, FontStyle.Regular);

            //centers the textbox text
            txtboxGameTimer.TextAlign = HorizontalAlignment.Center;

            //removes the textbox border
            txtboxGameTimer.BorderStyle = BorderStyle.None;

            //adds the textbox on the screen
            this.Controls.Add(txtboxGameTimer);

            //******************GETS IMAGES**********************

            //Gets the background image
            background = Image.FromFile(Application.StartupPath + @"\background.jpeg", true);
            ...
            platforms = Image.FromFile(Application.StartupPath + @"\platforms.png", true);
            ...
            rectMoneyBags[4] = new Rectangle(380, 358, moneyBagsWidth, moneyBagsHeight);
            rectMoneyBags[5] = new Rectangle(1740, 358, moneyBagsWidth, moneyBagsHeight);
            rectMoneyBags[6] = new Rectangle(40, 515, moneyBagsWidth, moneyBagsHeight);
            ...
            rectMoneyBags[9] = new Rectangle(1159, 782, moneyBagsWidth, moneyBagsHeight);
            ...
            {
                //initializes the file reader variable
                fileReader = new StreamReader(Application.StartupPath + @"\highscore.txt");

                //Runs while the file reader has not reached the end of the file
                while (fileReader.Peek() >= 0)
                {
                    //stores information in the highscore variable
            ...
            //adds text to the textbox
            txtboxHighscore.Text = "Highscore: " + highscore;
        ...
        //creates the display timer method
        private void DisplayTimer()
        {
            //adds to the time refreshed everytime the timer ticks
            timeRefreshed++;
            
            //runs if timerefreshed/ 60 gives no remainder
            if (timeRefreshed % 30 == 0 )
            {
                //reduces game timer by 1
                gameTimer--;

                //adds one to time passed
                timePassed++;

                //displays the timer
                txtboxGameTimer.Text = ("Timer: " + gameTimer);
            }
            //runs if game timer hits 0
            if (gameTimer == 0)
            {
                //stops the timer tick
                timerTick.Stop();

                //adds the picture box on the screen
                this.Controls.Add(picboxLose);
            }
        }
            ...
            for (int i = 0; i < rectMoneyBags.Length; i++)
            {
                ...
                if (bagsCollected == 10)
                {
                    if (timePassed < highscore)
                    {
                        ...
                        fileWriter.WriteLine(timePassed);
